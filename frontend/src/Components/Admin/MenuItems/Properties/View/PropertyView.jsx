...
} from "MyRedux/slices/PropertyValue";
...
import FormVED from "../../../../Tools/FormVED/FormVED";
...
function PropertyView(props) {
  ...
  const OnPropertySubmit = async (params) => {
    await dispatch(
      updateProperty({ ...params, type: typeId, category: categoryId })
    );
    dispatch(fetchProperties(propertiesData.currentPage));
    setOpen(false);
  };
