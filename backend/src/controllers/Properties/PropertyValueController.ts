...
export const createPropertyValue = async(req: Request, res: Response) =>{
    ...
    try{
        ...
        const propertyValue = new PropertyValues();
...
export const getPropertyValues =  async (req:Request, res:Response) => {
    ...
    let propertyValues;
    if(!req.body.id)
        propertyValues = await PropertyValueRepository.createQueryBuilder("property_values").limit(itemsPerPage).offset((page - 1) * itemsPerPage).getMany();
    else
        propertyValues = 
        await PropertyValueRepository
        .createQueryBuilder("property_values")
        .leftJoinAndSelect("property_values.property", "property")
